...
def GetValidSubdirectory(path, callback):    
    callback(path)
    ...
    for folder in os.listdir(path):
        ...
        if os.path.isdir(valid_dir) and folder[0] != "." and folder not in ignore_folders:
            GetValidSubdirectory(valid_dir, callback)

def IncludeFlagsOfSubdirectory( flags, working_directory ):
  new_flags = flags
  GetValidSubdirectory(working_directory,
          lambda directory : new_flags.append(["-I", directory]))
...
def Settings( **kwargs ):
  ...
  if language == 'cfamily':
    ...
    new_flags = IncludeFlagsOfSubdirectory(flags, DIR_OF_THIS_SCRIPT)
